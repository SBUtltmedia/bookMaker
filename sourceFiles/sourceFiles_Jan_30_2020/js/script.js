...
syncPromises.push(progress.syncKey(pageContent.content,i)) 

}
}
$.when(...syncPromises).then(()=>defer.resolve())
...
progress.setKey(key, {status:status, grade:grade}).then(function(){for (var i = 0; i < state.chapters.length; i++) {
