...
function setupVideo(){
   ...
   AdobeEdge.loadComposition('tubs', 'EDGE-40418101', {
    scaleToFit: "both",
...
function replay(){
  var symbol = AdobeEdge.getComposition("EDGE-40418101").getStage().getSymbol("Stage");
...
if (position != timelength){
  if(symbol.isPlaying()){
    ...
    symbol.$("media22")[0].pause();
    show();
  }else{
  symbol.play(position);
  symbol.$("media22")[0].play();
  hide();
  }
}else{
  symbol.play(0);
  symbol.$("media22")[0].currentTime = 0;
  symbol.$("media22")[0].play();
  hide();
}
