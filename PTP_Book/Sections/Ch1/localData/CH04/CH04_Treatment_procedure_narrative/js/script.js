...
function setupVideo(){
   ...
   AdobeEdge.loadComposition('hydro', 'EDGE-35485452', {
    scaleToFit: "both",
...
function replay(){
  var symbol = AdobeEdge.getComposition("EDGE-35485452").getStage().getSymbol("Stage");
...
if (position != timelength){
  if(symbol.isPlaying()){
    ...
    symbol.$("media12")[0].pause();
    show();
  }else{
  symbol.play(position);
  symbol.$("media12")[0].play();
  hide();
  }
}else{
  symbol.play(0);
  symbol.$("media12")[0].currentTime = 0;
  symbol.$("media12")[0].play();
  hide();
}
