...
function setupVideo(){
   ...
   AdobeEdge.loadComposition('conduction', 'EDGE-24421047', {
    scaleToFit: "both",
...
function replay(){
  var symbol = AdobeEdge.getComposition("EDGE-24421047").getStage().getSymbol("Stage");
...
if (position != timelength){
  if(symbol.isPlaying()){
    ...
    symbol.$("mediasplice")[0].pause();
    show();
  }else{
  symbol.play(position);
  symbol.$("mediasplice")[0].play();
  hide();
  }
}else{
  symbol.play(0);
  symbol.$("mediasplice")[0].currentTime = 0;
  symbol.$("mediasplice")[0].play();
  hide();
}
